function kapal_kapuk_train(varargin)
%KAPAL_KAPUK_TRAIN Train an SSD detector on pascal VOC data
...
%   opts.modelOpts.sourceModel = 'mcn-mobilenet-v2' ;
  ...
  opts.dataOpts.getImdb = @getCombinedPascalKapukImdb ;
  opts.dataOpts.prepareImdb = @kapuk_prepareImdb ;
  ...
  steadyLR = 0.0001 ; gentleLR = 0.00001 ; vGentleLR = 0.00001 ;
  ...
  % reproduce the experiments in the original paper.
...
% ---------------------------------------------------
function [opts, imdb] = kapuk_prepareImdb(imdb, opts)
% ---------------------------------------------------
...
function imdb = getCombinedPascalKapukImdb(opts, varargin)
% Combine dataset of VOC2007, VOC2012, and KAPUK3700

    opts.includeDetection = true ;
    opts = vl_argparse(opts, varargin) ;

    % Combine VOC2007 and VOC2012
%     imdbVoc = getCombinedPascalImdb(opts);
    load('imdbVoc.mat', 'imdbVoc');
    
    % add meta information (inlcuding background class)
    imdb.meta = imdbVoc.meta;
    classIds = 1:numel(imdb.meta.classes) ;
    imdb.classMap = containers.Map(imdb.meta.classes, classIds) ;
    imdb.images.ext = 'jpg' ;
    imdb.meta.sets = {'train', 'val', 'test'} ;
    
    opts.kapukRoot = fullfile(opts.dataOpts.dataRoot, 'KAPUK3700' ) ;
    dataDirKapuk = fullfile(opts.dataOpts.dataRoot, 'KAPUK3700', 'kapal_training') ;
    imdbKapuk = kapukSetup('dataDir', dataDirKapuk);
    
    % Combine voc imdb with kapuk imdb
    % ------------------------------------------------
    imdb.images.name = [imdbVoc.images.name, imdbKapuk.images.name] ;
    imdb.images.set = [imdbVoc.images.set imdbKapuk.images.set] ;
    imdb.images.year = [imdbVoc.images.year ...
                      ones(1,numel(imdbKapuk.images.name)) * 2019] ;
    imdb.images.classification = [imdbVoc.images.classification ...
                                imdbKapuk.images.classification] ;

    % add individual image paths for backward compatibility
    paths = repmat(imdbKapuk.paths.image, numel(imdbKapuk.images.name), 1) ;
    kapukPaths = arrayfun(@(x) {paths(x,:)}, 1:size(paths, 1)) ;
    imdb.images.paths = [imdbVoc.images.paths kapukPaths];

    % also include the paths from both years to handle optional extra segmentations
    imdb.pathsKapuk3700 = imdbKapuk.paths ;

    % for consistency, store in Height-Width order
    imageSizes = arrayfun(@(x) {imdbKapuk.images.size([2 1],x)'}, ...
                                      1:size(imdbKapuk.images.size, 2)) ;
    imdb.images.imageSizes = [imdbVoc.images.imageSizes imageSizes];
    if opts.includeDetection
        annotKapuk = kapuk_loadAnnotations(imdbKapuk, opts) ;
        imdb.annotations = horzcat(imdbVoc.annotations, annotKapuk) ;
    end

%     % Without imdb Kapuk dataset
%     % ------------------------------------------------
%     imdb.images.name = [imdbVoc.images.name] ;
%     imdb.images.set = [imdbVoc.images.set] ;
%     imdb.images.year = [imdbVoc.images.year] ;
%     imdb.images.classification = [imdbVoc.images.classification] ;
% 
%     % add individual image paths for backward compatibility
%     imdb.images.paths = [imdbVoc.images.paths];
% 
%     imdb.images.imageSizes = [imdbVoc.images.imageSizes];
%     if opts.includeDetection
%         imdb.annotations = imdbVoc.annotations ;
%     end
    
% ------------------------------------------------
function annotations = kapuk_loadAnnotations(imdb, opts)
% ------------------------------------------------
